...
renderPlot({
  eliminator_by_round %>% 
    left_join(player_count_by_game ,by = c("game_num")) %>%
    filter(player_count == input$PlayerCountInputPage3) %>% 
    group_by(eliminator ,round_loser) %>% 
    summarise(dice_eliminated = n()) %>% 
    ungroup() %>% 
    filter(eliminator == input$PlayerInput) %>%
    ggplot(aes(x = reorder(round_loser ,dice_eliminated) ,y = dice_eliminated)) +
      geom_col() +
      coord_flip() +
      theme_fivethirtyeight()
})
...
renderPlot({
  eliminator_by_round %>% 
    ...
    filter(player_count == input$PlayerCountInputPage3) %>% 
    filter(player == input$PlayerInput) %>% 
    group_by(player ,round_loser ,accusation_type) %>% 
    summarise(accusations      = n()
              ,dice_eliminated = sum(as.numeric(player != round_loser))
              ) %>% 
    ungroup() %>% 
    mutate(accusation_type2 = factor(if_else(accusation_type == "accuser" ,"Lies Called" ,"Called Liar")
                                     ,levels = c("Lies Called" ,"Called Liar"))
           ) %>% 
    filter(player != round_loser) %>%
    group_by(player ,accusation_type2) %>% 
    summarise(dice_eliminated = sum(dice_eliminated)) %>% 
    ungroup() %>% 
    ggplot(aes(x = reorder(accusation_type2 ,dice_eliminated) ,y = dice_eliminated)) +
      geom_col() +
      coord_flip() +
      theme_fivethirtyeight()
})
...
### Success Rate by Method & Opponent
...
renderPlot({
  eliminator_by_round %>% 
    ...
    ungroup() %>% 
    filter(player == input$PlayerInput) %>%
    ggplot(aes(x = reorder(opponent ,success_rate) ,y = success_rate)) +
      geom_col() +
      coord_flip() +
